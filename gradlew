...
#       will be used. Otherwise, java must be in your PATH.
#
##############################################################################
...
# Collect all arguments for the java command, stracks://issues.gradle.org/browse/GRADLE-2465
...
if ! "$cygwin" && ! "$msys" ; then
    case $( set -- $$; readlink -f /proc/$1/exe 2>/dev/null || readlink /proc/$1/exe 2>/dev/null ) in #(
      '')   :;; #(
      *busybox*)
            :;; #(
    esac
